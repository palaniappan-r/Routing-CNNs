...
def calculate_all(csv_path):
    ...
    first_flag = False
    idx_Int = 0
    print("Work for AUC of ROC...")
    for line in csv_file:
        idx_Int = idx_Int+1
        # print("idx=",idx_Int)
        if(0==idx_Int%2): #+Harley 2024/01/16
            continue #===> AUC of ROC. 0.9391
...
def multi_process_score(out_name=None, threshold=0.0, label_path=None, save_path=None):
    ...
    print('Copying the tpr_fpr.CSV file...')
    command_str ='copy {} {}'.format(os.path.join(temp_path, f'{out_name}'),
                                os.path.join(save_path, f'{out_name}'))
    command_str = command_str.replace(r"/", "\\")
    print(command_str)
    os.system(command_str)

    # os.system(f'rm -rf {temp_path}') #for Linux
    print('Remove the temporary files and the folder...')
    command_str =f'rmdir {temp_path}'
    command_str = command_str.replace(r"/", "\\")
    command_str = command_str+" /Q/S"
    print(command_str)
    os.system(command_str)
...
def roc_prc(save_path):
    print("ROC_PRC Working on ==> {} ".format(os.path.join(os.getcwd(), save_path, 'roc_prc.csv')))
    tpr_sum_List, fpr_sum_List, precision_sum_List = calculate_all(os.path.join(os.getcwd(), save_path, 'roc_prc.csv'))

    # print("ROC_PRC Working on ==> {} ".format(os.path.join(save_path, 'roc_prc.csv')))
...
def roc_prc(save_path):
    ...
    fileRoc = open("fileRoc.txt", "w")
    # ---roc_auc = metrics.auc(np.array(fpr_sum_List), np.array(tpr_sum_List))
    for kk in range(len(tpr_sum_List)):
        print(fpr_sum_List[kk],tpr_sum_List[kk])
        fileRoc.write("{0}, {1}\n".format(fpr_sum_List[kk],tpr_sum_List[kk]))
    fileRoc.close()
    # --Harley
    # plt.plot(fpr_sum_List, tpr_sum_List, 'bD-', label='model thresholds', linewidth=2);
...
def build_roc_prc_metric(threshold=0.0, dataroot=None, ann_file=None, save_path=None, **kwargs):
    # -- temp Harley 2024/01/16
    '''
    if ann_file:
    ...
    multi_process_score(out_name='roc_prc.csv',
                        threshold=threshold,
                        label_path=os.path.join(dataroot, label_name),
                        save_path=os.path.join('.', save_path))
    '''
    save_path = "ROC_CURVE_drc_routenet"
